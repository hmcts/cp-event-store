...
class StreamsResourceTest {
    ...
    @Mock
    private NewStreamStatusRepository repository;
    ...
    void shouldReturnSuccessResponseGivenStreamsFoundWithErrorHash() {
        ...
        final Response response = streamsResource.findBy(errorHash.toString());

        assertThat(response.getStatus(), is(200));
        final List<Stream> streams = (List<Stream>) response.getEntity();
        ...
        assertTrue(streams.get(0).errorId().isEmpty());
        assertTrue(streams.get(0).errorPosition().isEmpty());
        ...
        assertThat(streams.get(1).errorId().get(), is(streamStatus2.streamErrorId().get()));
        assertThat(streams.get(1).errorPosition().get(), is(streamStatus2.streamErrorPosition().get()));
    }
    ...
    void shouldReturnEmptyListGivenNoStreamsFoundWithErrorHash() {
        ...
        assertThat(response.getStatus(), is(200));
        final List<Stream> streams = (List) response.getEntity();
