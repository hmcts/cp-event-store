...
5 round-trips per batch vs 6 per event. For batch size 20: **120 round-trips reduced to 5**. The per-event overhead (JTA begin/commit, advisory lock acquire/release, connection management) is amortised across the batch.
...
| Class | Change |
|-------|--------|
| `EventNumberLinker` | Rewritten: `findAndLinkEventsInBatch()` returns `int` (count linked). Single connection, batch operations. |
| `EventLinkingWorker` | Loop calls `findAndLinkEventsInBatch()` until 0 returned or time exceeded. |
| `LinkEventsInEventLogDatabaseAccess` | New connection-accepting batch methods: `getEventStoreConnection()`, `findBatchOfNextEventIdsToLink(conn, size)`, `linkEventsBatch(conn, list)`, `insertBatchIntoPublishQueue(conn, list)`. Old self-connecting methods removed. |
...
| Metric | Original (per-event) | Batch (size 10) |
|--------|---------------------|-----------------|
